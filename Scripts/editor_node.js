var Node = function (id, positionX, positionY, zindex, color, title, typeName, owner, editedTypeName)
{
    ...
    this.origPositionY;
    this.zindex = zindex;
    this.color = color; //"transparent"; //color;
    this.title = title;
    ...
    this.GetHtml = function (style)
    {
        ...
        {
            html += "<div class='ed_title'><div class='ed_colorSelect'><input class='ed_colorpicker2' value='" + self.color + "' type='text'></input><script>$('.ed_colorpicker2').colorPicker({renderCallback: function(){ $('.ed_colorpicker2', '#" + self.id + "').trigger('colorChanged'); }});</script></div><span class='ed_nodeTitleText'>" + self.title + "</span><div class='ed_nodeButtons'><div class='ed_saveNodeType ed_nodeButton'>Uložit</div><div class='ed_renameNode ed_nodeButton'>Přejmenovat</div><div class='ed_cancelNodeType ed_nodeButton'>Zrušit</div></div></div>"
        }
        else
        {
    ...
    {
        if (!nodeElement) nodeElement = $("#" + self.id);
        return nodeElement;
    };
    ...
    this.RefreshValues = function (initial)
    {
        for (var i = 0; i < self.contentItems.length; i++)
        {
            self.contentItems[i].RefreshValue(initial);
        }
    }
    ...
    {
        $(".ed_selectGroup", self.GetElement()).html(GroupsHtml());
    }
    ...
    this.GetConnectionOutsideGroup = function ()
    {
        var ret = []
        ...
        else
        {
            ...
            {
                for (var i = 0; i < self.contentItems[index].connections.length; i++)
                {
                    var connectedItem;
                    if (self.contentItems[index].type == "output") connectedItem = self.contentItems[index].connections[i].contentItemInput;
                    else connectedItem = self.contentItems[index].connections[i].contentItemOutput;

                    if (!connectedItem.owner.group || connectedItem.owner.group.name != self.group.name) ret.push(self.contentItems[index].connections[i]);
                }
            }
        }
        return ret;
